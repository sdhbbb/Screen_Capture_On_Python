# 실행시 백그라운드에서 동작하게 하기 위해 콘솔창을 보여주지 않는 pyw 확장자 사용
# 필요한 모듈을 import 하는 부분

# 운영체제와의 상호 작용을 위한 모듈
import os
# 시스템과 관련된 기능, 코드들을 사용하기 위한 모듈
import sys
# JSON에 설정을 저장하고 불러오기 기능을 사용하기 위한 모듈
import json
# 키보드의 입력을 받고 처리하는 기능을 위한 모듈
import keyboard
# 입력을 통해 스크린샷을 저장하기 위한 모듈
import pyautogui
...
# 2. 해당 클래스는 설정창(UI)을 구현하기 위한 클래스이다.
class SettingsWindow(QWidget):
    def __init__(self, screenshot_app=None):
        ...
        # 창을 닫으면 프로그램은 종료되지만, 해당 코드를 통해 창을 닫아도 프로그램이 종료되지 않도록 한다
    ...
    def init_ui(self):
        ...
        # 설정 저장 버튼을 만들기 위한 코드
        # Save Settings 버튼을 추가한다.
        self.save_button = QPushButton("Save Settings")
        # Save Settings 버튼 클릭시 현재 UI에서 지정되었던 설정을 저장한다
        self.save_button.clicked.connect(self.save_settings)
        # Save Settings 위젯(클릭할 수 있는 버튼)을 추가하는 부분이다.
        layout.addWidget(self.save_button)
        
        # 레이아웃 설정을 위한 코드
        self.setLayout(layout)
        # 켜져있는 UI의 프로그램 제목을 설정하기 위해 작성하였다.
    ...
    def browse_save_path(self):
        ...
        # 저장 경로 지정을 더 편하게 할 수 있도록 다이얼로그를 띄워주는 부분
        path, _ = QFileDialog.getSaveFileName(
            self,
            # 다이얼로그로 열린 창의 이름을 Select Save Path로 지정
            "Select Save Path",
            self.screenshot_app.save_path,
            # 저장할 이름을 띄워주는 부분이다. 현재는 파일의 저장명을 screenshot_연월일_시분초로 강제로 지정하고있다.
            "image names (*.png)"
        )
        # 만약, 경로가 설정되었을 경우 경로를 확인하기 위해서
        if path:
    ...
    def save_settings(self):
        # 입력되어 있는 핫키를 new_hotkey 변수에 가져온다.
        ...
        if self.screenshot_app:
            # 새로운 핫키와 저장경로를 업데이트한다
            self.screenshot_app.update_settings(new_hotkey, new_save_path)
        
        # 저장이 정상적으로 완료되었는지 확인하기 위해 알림으로 저장이 성공적으로 완료되었다는 것을 안내한다.
        QMessageBox.information(self, "Settings Saved", "Your settings have been saved successfully.")
        
        # 저장이 완료되었으니 창을 숨기는 코드이다.
        self.hide()

    # 창 닫기 요청이 들어왔을 때 프로그램을 종료시키지 않기 위한 방지용 코드이다.
    # 프로그램 종료가 아닌, 창을 숨기는 방식으로 계속 프로그램을 작동시키도록 한다.
...
class ScreenshotApp:
    # 설정을 저장할 파일의 이름을 지정한 부분이다, 따로 작성한 파일이 있으면 수정하여주면 된다.
    SETTINGS_FILE = "settings.json"

    def __init__(self, app):
        # Qapplication의 객체이다.
        self.app = app
        # 프로그램 처음 시작 시, 기본으로 지정해둔 핫키이다, 기본적인 설정이며, settings.json에서 불러올 것이기에 큰 의미는 없다.
        self.hotkey = 'ctrl+alt+c'
        # 기본으로 지정된 저장 경로이다, 핫키와 마찬가지이다.
        self.save_path = os.path.expanduser("~/Desktop/screenshot.png")
        # 아이콘으로 지정할 파일의 이름이다.
        self.icon_path = "SCOP.ico"
        ...
        # 트레이 메뉴를 만든다.
        tray_menu = QMenu()
        # 트레이 아이콘을 우클릭 후 Settings란에 접근 시, UI가 켜져서 원하는 세팅을 할 수 있도록 메뉴를 추가해 준다
        settings_action = tray_menu.addAction("Settings")
        # 클릭시 설정 창을 열수 있게 연결해 주는 트리거이다.
        settings_action.triggered.connect(self.show_settings)
        # 종료를 하기 위해 Exit 버튼을 추가해주는 코드이다. 마찬가지로 트레이 아이콘을 우클릭시 접근 가능하다
        ...
        # 예시로 프로그램이 바로 종료되거나, 콘솔창에 트레이 아이콘을 찾을 수 없다는 오류가 뜨기도 하였다.
    ...
    def load_settings(self):
        # 설정 파일(settings.json)이 존재여부를 체크
        if os.path.exists(self.SETTINGS_FILE):
            ...
            try:
                ...
                # 파일이 존재하지 않거나, 파일 깨짐등의 오류로 인해 읽어들일 수 없다
                QMessageBox.warning(None, "Settings Error", f"Failed to load settings: {e}")

    # settings.json에 작성된 세팅을 저장하기 위한 부분
    def save_settings_to_file(self):
        ...
        try:
            ...
            with open(self.SETTINGS_FILE, "w") as file:
                # JSON파일이므로 json의 형식으로 저장한다(들여쓰기는 보통 4이므로 indent=4를 지정)
                json.dump(settings, file, indent=4)
        # 저장실패, 파일 오류와 같은 경우에 안내할 메시지
        except Exception as e:
            # 저장에 문제가 생겼다고 사용자에게 안내하는 메시지 표기이다
    ...
    def register_hotkey(self):
        ...
        # 예외적인 상황으로 핫키 등록에 실패 시 표기할 메시지 
            ...
            # ui를 불러올 수 없던 환경이거나, 핫키가 고정되었을 경우 에러가 나타난 적이 있어서 추가
    ...
    def handle_screenshot_result(self, result):
        ...
        if result.startswith("Error"):
            # Screenshot Error가 발생했다는 에러를 표기한다.
    ...
    def exit_application(self):
        ...
        # 만약 YES를 클릭할 경우, 프로그램을 종료하게 한다
        # 해당 프로그램이 종료되는 경우는 Exit로만 종료된
